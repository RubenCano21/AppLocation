...
- "api-layout": a directory representing a Python project that contains a "api"
  folder. Everything under the "api" folder is meant to be included in the
...
- "flat-layout": a Python project that does not use "api-layout" but instead
...
class ConfigDiscovery:
    ...
    def _analyse_explicit_layout(self) -> bool:
        ...
        package_dir.pop("", None)  # This falls under the "api-layout" umbrella
    ...
    def _analyse_src_layout(self) -> bool:
        """Try to find all packages or modules under the ``api`` directory
        (or anything pointed by ``package_dir[""]``).

        The "api-layout" is relatively safe for automatic discovery.
        We assume that everything within is meant to be included in the
        distribution.

        If ``package_dir[""]`` is not given, but the ``api`` directory exists,
        this function will set ``package_dir[""] = "api"``.
        """
        package_dir = self._package_dir
        src_dir = os.path.join(self._root_dir, package_dir.get("", "api"))
        if not os.path.isdir(src_dir):
            return False

        log.debug(f"`api-layout` detected -- analysing {src_dir}")
